...
// PC: program counter, it stores an memory address in which stores the next instruction to be run.
// SP: stack pointer, which always points to the top of the stack. Notice the stack grows from high address to low address so that when we push a new element to the stack, SP decreases.
// BP: base pointer, points to some elements on the stack. It is used in function calls.
...
// for lexical analysis; get the next token; it will ignore spaces, tabs etc.
void next() {
    char *last_pos;
    int hash;
    while (token = *src) {
